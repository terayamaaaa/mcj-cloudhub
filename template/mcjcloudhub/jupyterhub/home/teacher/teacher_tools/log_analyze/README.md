...
※ この分析ツールでは、[`Jupyter-LC_wrapper`](https://github.com/NII-cloud-operation/Jupyter-LC_wrapper), [`Jupyter-LC_nblineage`](https://github.com/NII-cloud-operation/Jupyter-LC_nblineage)によって出力される実行履歴情報を利用します。教師・学生環境いずれも、これらのjupyter拡張が有効になっている必要があります。  
...
学生一覧はnbgraderのDB（`gradebook.db`）から取得するため、nbgrader上の受講生として登録されているユーザ（Moodleにて、そのコースに登録されているユーザ）が対象となります。  
...
「`assignment`, `student_id`, `cell_id`, `log_sequence`」の組に対してユニーク制約を設定しています。
...
|   6 |                                | notebook_name                  | VARCHAR()                      |          |                      |ノートブック名                                |
|   7 |                                | log_json                       | JSON                           |          |                      |ログファイル（.json）全体         |
|   8 |                                | log_code                       | VARCHAR()                      |          |                      |セルの内容                                |
|   9 |                                | log_path                       | VARCHAR()                      |          |                      |ログファイルのパス                          |
|  10 |                                | log_start                      | TIMESTAMP                      |          |                      |実行開始日時                                |
|  11 |                                | log_end                        | TIMESTAMP                      |          |                      |実行終了日時                                |
|  12 |                                | log_size                       | INTEGER                        |          |                      |ログファイルのサイズ                                |
|  13 |                                | log_server_signature           | VARCHAR()                      |          |                      |サーバーの署名ID                                |
|  14 |                                | log_uid                        | INTEGER                        |          |                      |ユーザのUID                                |
|  15 |                                | log_gid                        | INTEGER                        |          |                      |ユーザのGID                                |
|  16 |                                | log_notebook_path              | VARCHAR()                      |          |                      |実行したノートブックのパス                                |
|  17 |                                | log_lc_notebook_meme           | VARCHAR()                      |          |                      |実行したノートブックのmeme_id                                |
|  18 |                                | log_execute_reply_status       | VARCHAR()                      |          |                      |実行結果 'ok'/'error'                                |
...
|   6 |                                | nbgrader_cell_id               | VARCHAR()                      |          |                      |nbgraderのcreate assignment機能で設定するセルのID                                |
